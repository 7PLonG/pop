    ...
    $.extend({
        ...
        // 函数主体
            ...
            function stopscroll(){
                ...
                $('body').css({'overflow':'hidden','padding-top':1,
                    'position':'fixed','left':0,'right':0,
                    'top':(-1*scrollvalue+'px')});
                // $('body').css({'overflow':'hidden','padding-top':(-1*scrollvalue+'px')});
            }
            function rescroll(){
                // document.removeEventListener('touchmove',function(e){
                //     e.preventDefault();
                // },false);
                $('body').css({'overflow':'','padding-top':'','position':'','top':'','left':'','right':''});
                $(window).scrollTop(scrollvalue);
                // 'position':'relative'配合top值 PC上有显示问题，保留修改为fixed
            }
            function popRemove() {
                if (stopScroll){
                    rescroll();
